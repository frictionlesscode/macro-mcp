...
Copied from garmin-mcp's oauth.py (SPEC.md M5, "OAuth provider copied from garmin-mcp") with
only branding and the state-file default changed -- the security-relevant logic is identical
on purpose, and was already exercised as a *client* of this exact class against the real,
running garmin-mcp instance during M4 (register/authorize/token-exchange/refresh all verified
live). Running the same code as the auth server here means that verification transfers.
...
from macro_mcp.auth import client_key, limiter
...
class SingleUserOAuthProvider(InMemoryOAuthProvider):
    ...
    the `/data` volume (already mounted for the SQLite store) and reloading
    it on startup. Expired entries are dropped on load rather than carried
    forward.
